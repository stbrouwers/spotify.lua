...
local function Refresh()
    ...
    http.get("https://api.spotify.com/v1/me?market=GB&access_token="..authkey, function(success, response)
        if not success or response.status ~= 200 then client.error_log("Failed to authenticate with Spotify servers."); IsNotAuthed = true; return end
...
local getkey = ui.new_button("MISC", "Miscellaneous", "Get auth key from clipboard", GetAuthKeyFunc)
local auth = ui.new_button("MISC", "Miscellaneous", "Authorise", Refresh)

local last_update = client.unix_time()
client.set_event_callback("paint_ui", function()
    ui.set_visible(auth, IsNotAuthed)
    if spotidata == nil then return end
    if spotidata2 == nil then return end
    ...
    r, g, b, a = ui.get(col)
    renderer.gradient(0, 70, textmeasurement, 32, 22, 22, 22, 255, 22, 22, 22, 10, true)
